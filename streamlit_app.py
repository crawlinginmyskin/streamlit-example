...
test_t = tuple(time(int(test[i][0]), int(test[i][1])) for i in range(len(test)))
...
if byla_jazda and choice is not None:
	...
	if not is_rf:
		...
		with st.expander("Wpływ parametrów wejsciowych na pojedyncze predykcje"):
			...
			n = st.slider('Wybierz obserwację do analizy', test_t[0], test_t[-1], test_t[0], step=timedelta(minutes=60))
